...
- (vector3f)intersectWithLineStart:(vector3f)startPoint end:(vector3f)endPoint // TODO: this is completely buggy, but i suspect the tri-ray code is faulty
{
	...
	if (rotation[0] || rotation[1] || rotation[2])
	{
		...
		matrix_rotation_euler(rm, cml::rad(-rotation[0]), cml::rad(-rotation[1]), cml::rad(-rotation[2]), cml::euler_order_xyz);
...
- (TriangleIntersectionInfo)intersectWithNode:(CollideableMesh *)otherMesh
{
	matrix33f_c rm, orm;
	TriangleIntersectionInfo tif;

	matrix_rotation_euler(rm, cml::rad(rotation[0]), cml::rad(rotation[1]), cml::rad(rotation[2]), cml::euler_order_xyz);
	matrix_rotation_euler(orm, cml::rad([otherMesh rotation][0]), cml::rad([otherMesh rotation][1]), cml::rad([otherMesh rotation][2]), cml::euler_order_xyz);
...
{
	if (thisOctree->vertexCount > 0xFFFF || otherOctree->vertexCount > 0xFFFF)
		return NO;
	...
	{
		vector3f aabbExtent1 = vector3f(n1->aabbExtentX, n1->aabbExtentY, n1->aabbExtentZ) / 2.0f;
		vector3f aabbOrigin1 = vector3f(n1->aabbOriginX, n1->aabbOriginY, n1->aabbOriginZ) + aabbExtent1 + position;
		vector3f aabbExtent2 = vector3f(n2->aabbExtentX, n2->aabbExtentY, n2->aabbExtentZ) / 2.0f;
...
int tri_tri_overlap_test_2d(float p1[2], float q1[2], float r1[2],
		float p2[2], float q2[2], float r2[2])
{
	if (ORIENT_2D(p1, q1, r1) < 0.0f) if (ORIENT_2D(p2, q2, r2) < 0.0f)

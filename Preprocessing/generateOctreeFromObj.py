...
try:
	...
	if of == 0:											of = open(sys.argv[len(sys.argv) - 1][:sys.argv[len(sys.argv) - 1].rfind(".")] + ".octree", 'w')
except:
...
for line in lines:
	...
	if i == "v":
		...
		for d in range(1, len(c) - 1):
			faces.append([	int(faceContent(c[0],0))-1, int(faceContent(c[d],0))-1, int(faceContent(c[d+1],0))-1,
							int(faceContent(c[0],2))-1, int(faceContent(c[d],2))-1, int(faceContent(c[d+1],2))-1,
							int(faceContent(c[0],1))-1, int(faceContent(c[d],1))-1, int(faceContent(c[d+1],1))-1])	
...
for face in faces:
	for i in range(3):
		if face[i+3] == -1:
			normalwarning += 1
			normals.append(normalize(crossProduct(substract(vertices[face[1]],vertices[face[0]]), substract(vertices[face[2]],vertices[face[0]]))))
